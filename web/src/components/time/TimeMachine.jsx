...
export default function TimeMachine() {
  ...
  const setTmPlay  = useDashboard((s) => s.setTmPlaying)
  const [playing, setPlaying] = useState(false)
  const timer = useRef(null)

  const idx = Math.max(0, TIMELINE_YEARS.indexOf(Number(year)))

  // sync play state to store so SpectralPanel can react
  useEffect(() => { setTmPlay(playing) }, [playing, setTmPlay])
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <input
        type="range"
        min={0} max={TIMELINE_YEARS.length - 1} step={1} value={idx}
        onChange={(e) => {
          setPlaying(false)
